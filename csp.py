    ...
    for B in csp.neighbors[var]:
        if B not in assignment:
            for b in csp.curr_domains[B][:]:
                # Έλεγχος αν η ανάθεση var=value ικανοποιεί όλους τους σχετικούς περιορισμούς
                assignment[B] = b
                assignment[var] = value
                if not csp.consistent(assignment):
...
def backtracking_search(csp, select_unassigned_variable=first_unassigned_variable,
                        ...
                        inference=no_inference, max_nodes=float('inf')):
    """Backtracking search with node expansion limit."""

    def backtrack(assignment, node_count):
        if node_count[0] >= max_nodes:  # Check if node limit has been reached
            return None

        if len(assignment) == len(csp.variables):  # If assignment is complete
            return assignment

        var = select_unassigned_variable(assignment, csp)
        for value in order_domain_values(var, assignment, csp):
            if 0 == csp.nconflicts(var, value, assignment):  # Check for conflicts
                csp.assign(var, value, assignment)
                removals = csp.suppose(var, value)
                node_count[0] += 1  # Increment node counter

                if inference(csp, var, value, assignment, removals):
                    result = backtrack(assignment, node_count)
    ...
    result = backtrack({}, [0])  # Initialize node counter
    return result  # Return only the solution
...
class NaryCSP:
    ...
    neighbors   : a variable to set of neighboring variables dictionary
    """

    def __init__(self, domains, constraints):
        """Domains is a variable:domain dictionary. Constraints is a list of constraints."""
        self.variables = set(domains)
        self.domains = domains
        self.constraints = constraints

        # Αντιστοίχιση μεταβλητών σε περιορισμούς
        self.var_to_const = {var: set() for var in self.variables}
        for con in constraints:
            for var in con.scope:
                self.var_to_const[var].add(con)

        # Δημιουργία neighbors: Μεταβλητή -> Γείτονες
        self.neighbors = {var: set() for var in self.variables}
        for con in constraints:
            for var in con.scope:
                self.neighbors[var].update(v for v in con.scope if v != var)

        self.curr_domains = None  # Για forward checking

    def consistent(self, assignment):
        """Επιστρέφει True αν όλοι οι σχετικοί περιορισμοί είναι ικανοποιημένοι."""
        return all(con.holds(assignment)
                   for con in self.constraints
                   if all(v in assignment for v in con.scope))

    def support_pruning(self):
        """Αρχικοποιεί το curr_domains αν δεν είναι ήδη αρχικοποιημένο."""
        if self.curr_domains is None:
            self.curr_domains = {var: list(self.domains[var]) for var in self.variables}

    def suppose(self, var, value):
        """Προσωρινά αναθέτει μια τιμή και επιστρέφει τις αλλαγές (removals)."""
        self.support_pruning()
    ...
    def prune(self, var, value, removals):
        """Αφαιρεί μια τιμή από το πεδίο τιμών μιας μεταβλητής."""
        self.curr_domains[var].remove(value)
        removals.append((var, value))

    def restore(self, removals):
        """Επαναφέρει τις τιμές που είχαν αφαιρεθεί από τα πεδία τιμών."""
        for var, value in removals:
            if value not in self.curr_domains[var]:
                self.curr_domains[var].append(value)

    def choices(self, var):
        """Επιστρέφει τις διαθέσιμες τιμές για μια μεταβλητή."""
        return self.curr_domains[var] if self.curr_domains else self.domains[var]

    def assign(self, var, value, assignment):
        """Αναθέτει μια τιμή στη μεταβλητή var."""
        assignment[var] = value

    def unassign(self, var, assignment):
        """Ακυρώνει την ανάθεση μιας μεταβλητής."""
        if var in assignment:
            del assignment[var]

    def nconflicts(self, var, value, assignment):
        """Υπολογίζει τον αριθμό των παραβιάσεων περιορισμών για την τιμή value της μεταβλητής var."""
        assignment[var] = value
        conflicts = 0
        for con in self.var_to_const[var]:
            if all(v in assignment for v in con.scope) and not con.holds(assignment):
                conflicts += 1
        del assignment[var]
        return conflicts

    def goal_test(self, assignment):
        """Ελέγχει αν το assignment ικανοποιεί όλους τους περιορισμούς."""
        return len(assignment) == len(self.variables) and self.consistent(assignment)
